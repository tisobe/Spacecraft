PRO DTREND_DATE_FILT, infile, SIG=sig, WSMOOTH=wsmooth, PLOTX=plotx, $
            OUTDIR=outdir, FILT_IT=filt_it, XAX=xax, $
            AVG_ONLY=avg_only, QTR_ONLY=qtr_only, CCODE=ccode

; this is special, hack version to plot the last month 
;     see line 60,158    bds 12.jun 03
...
pred_days=0 ; number of days to look ahead for predicted violations
...
;testfor n_if_time = 1, 3 do begin ; loop to do daily min/max
  ...
  while (loop le loop_end) do begin ; this is the loop to do several
    ...
    data=data(where(data.time gt 170208063)) ; save only since May 25,2003
    save_groot=groot
    ...
    if (xaxtime ne 1) then xlim = limits(xmsid,describe=xdes,unit=xunits)
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      if (xmsid ne msid) then begin
        ...
        if (num gt 1) then begin
          ...
          ;xtime = [xdmin,xdmax]
          xtime = [0,1]
          xtarr = [min(data.time),max(data.time)]
          xdata = [0,0]
          xcolr = [white,white]
        endelse
        ...
        if (keyword_set(ccode)) then begin
          ...
          ; !p.multi = [0,1,2,0,0]
          !p.multi = [0,1,1,0,0] ; date_filt - deriv no good
          xrmarg = 6
        endelse ; if (keyword_set(ccode)) then begin
        xlmarg=10
        ...
        if (xaxtime eq 1) then begin
        ;date_filt  plot, xtime, dv*speryr, $
        ;date_filt      xstyle=1,ystyle=1, psym=symbol, symsize=ssize, $
        ;date_filt      title=strcompress("dy/dx "+msid), $
        ;date_filt      ;xtitle='UT (DOY)', ytitle=units+'/Year', $
        ;date_filt      xtitle='Time (UT)', ytitle=units+'/Year', $
        ;date_filt      xrange=[xmin,xmax], yrange=[ymin,ymax], $
        ;date_filt      charsize=pcsize,charthick=1,color=white,background=bgrd, $
        ;date_filt      xticks=nticks-1, xtickv = doyticks, xminor=10, $
        ;date_filt      xtickformat='s2doy_axis_labels', xmargin=[xlmarg,xrmarg]
        ;date_filt      ;xtickformat='s2cal_axis_labels', xmargin=[xlmarg,xrmarg]
        ...
        ;date_filt      xstyle=1,ystyle=1, psym=symbol, symsize=ssize, $
        ;date_filt      title=strcompress("dy/dx "+msid), $
        ;date_filt      xtitle= xmsid+" ("+xunits+")", $
        ;date_filt      ytitle=units+'/'+xunits, $
        ;date_filt      xrange=[xmin,xmax], yrange=[ymin,ymax], $
        ;date_filt      charsize = pcsize, charthick =1, $
        ;date_filt      color = white, background = bgrd, xmargin=[xlmarg,xrmarg]
        ;date_filtplots, xtime,dv*speryr,color=ccolor,psym=symbol,symsize=ssize
        line = (ddfit(1)*xtime+ddfit(0))*speryr
        ;date_filtoplot, xtime, line, color=blue
