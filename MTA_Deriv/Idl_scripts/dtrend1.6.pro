...
for i = 0, n_elements(keys)-1 do begin
  ...
  x = where(strpos(string(data.(j)), 'NaN') eq -1 and data.(j) ne -99, num)
...
; write html output
dtrend_html, xdmin, xdmax, html, htmlout, rootname
...
for i = 0, numlns-1 do begin
  if (strpos(array(i), '<!-- start last updated -->') ge 0) then begin
    ...
    printf, ounit, 'DOM '
    printf, ounit, strtrim(string(fix(cxtime(xdmin,'sec','met'))),2)
    printf, ounit, '</td>'
    printf, ounit, '<td align=right>Data end: '
    printf, ounit, strtrim(cxtime(xdmax,'sec','cal'),2)
    printf, ounit, 'DOM '
    printf, ounit, strtrim(string(fix(cxtime(xdmax,'sec','met'))),2)
    printf, ounit, '</td>'
    printf, ounit, '</tr>'
    printf, ounit, '</table>'
  ...
  if (data_sec eq 0) then begin  ; not in data section, so copy old file
    if (strpos(array(i), rootname+".html") ge 0) then begin
      ...
      if (ymark gt 0 and rmark le 0) then $
        printf, ounit, '<font color="'+hyellow+'">'+strmid(array(i),lstart,200)
      if (rmark gt 0) then $
        printf, ounit, '<font color="'+hred+'">'+strmid(array(i),lstart,200)
      if (ymark le 0 and rmark le 0) then $
        printf, ounit, '<font color="'+hwhite+'">'+strmid(array(i),lstart,200)
