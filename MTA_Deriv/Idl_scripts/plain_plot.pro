PRO plain_plot, infile, SIG=sig, WSMOOTH=wsmooth, PLOTX=plotx, $
...
set_plot, 'ps'
device, filename='xtrend.ps',xsize=20, ysize=15, $
         /encapsul,/color
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      if (xaxtime eq 1) then $
        plot, xtime, xdata, psym=symbol, ystyle=1, symsize=ssize, $
            ...
            charsize=1.0, charthick=2.0, thick=2,color = white, background = bgrd, $
      ...
      if (xaxtime eq 0) then $
        plot, xtime, xdata, psym=symbol, ystyle=1, symsize=ssize, $
            ...
            ytitle=msid+" ("+units+")", $
            xrange=[xmin,xmax], yrange=[ymin,ymax], ylog=setlog, $
            charsize=1.0, charthick=2.0, color = white, background = bgrd
...
lim=[0,0,0,0,0,0,0]
      ...
      if (xaxtime eq 1) then begin
        ...
        label_year, xmin, xmax, ycon(1), csize=1.0
      endif
      if (xaxtime eq 0) then $
        plot, xtime, dv*speryr, ystyle=1, psym=symbol, symsize=ssize, $
            title=strcompress("dy/dx "+msid), $
