...
wk_start=cxtime('2009-07-13T00:00:00','cal','sec')
wk_end=cxtime('2009-07-21T00:00:00','cal','sec')
...
if (strmatch(infile,'*_att.fits')) then ifile=outdir+"/pitch.html" else ifile=outdir+"/index.html"
if (strmatch(infile,'*_alt.fits')) then ifile=outdir+"/altitude.html"
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      if (xmsid ne msid) then begin
        ...
        if (xaxtime eq 0) then begin
          ...
          if (xax eq "sc_altitude") then begin
            setxlog="yes"
            setxlog="no"
            xtime=xtime/1000.
            xdes="Altitude"
        ...
        if (xaxtime gt 0) then begin
          ...
          case xax of  ; non-time ordinate
            ...
            'sc_altitude': $
              spawn,"perl /data/mta4/Deriv/Script/Idl_scripts/find_moving_avg.perl env_dat.in 5000 4 env_dat.out"
            else: begin
...
print, "ws ",wsmooth,ws
        ...
        endelse
        ; test
        ymin = min(xsave_dv*speryr)-0.10*abs(r)
        ymax = max(xsave_dv*speryr)+0.10*abs(r)
