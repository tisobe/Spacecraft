PRO DTREND_WK, infile, SIG=sig, WSMOOTH=wsmooth, PLOTX=plotx, $
            OUTDIR=outdir, FILT_IT=filt_it, XAX=xax, $
            AVG_ONLY=avg_only, QTR_ONLY=qtr_only, WK_ONLY=wk_only, $
            YR_ONLY=yr_only, CCODE=ccode
; this version will only make weekly trends vs. pitch plots and data tables
;   (some keywords are obsolete)
; temp 04/15/09 - this needs to be more automated
wk_start=cxtime('2010-08-16T00:00:00','cal','sec')
wk_end=cxtime('2010-08-23T00:00:00','cal','sec')
alt=mrdfits('pt_suncent_wk.fits',1)
tmp_out="att_"+infile
tmp_save,infile,tmp_out,n_out=-1,merge='pt_suncent_wk.fits'

;rdfloat,'/data/mta/DataSeeker/data/repository/earth_rad_ang.rdb', $
;         etime,tot_ang,p_ang,r_ang,skipline=2
wk_only=1
if (not keyword_set(outdir)) then outdir="/data/mta4/www/DAILY/mta_deriv/WK"
;;;;;;;
...
if (keyword_set(wk_only)) then nlstart=4
if (keyword_set(yr_only)) then nlstart=5

; force only weekly
nlstart=4
ndaily=4
 
frame=["AVG","MAX","MIN","QTR","WK","YR"]
...
;testfor n_if_time = 1, 3 do begin ; loop to do daily min/max
  ...
  while (loop le loop_end) do begin ; this is the loop to do several
    ...
    if (strpos(tnames(xcol),"_") eq 0) then s=1 else s=0
    ...
    if (n_if_time eq 4) then begin
      ;qtr=where(data.time ge max(data.time)-8*sperdy_ref)
      ;qtr=where(data.time ge wk_start and data.time le wk_end)
      ;print,n_elements(data),max(data.time), sperdy_ref, qtr(0) ; debug qtr
      ;data=data(qtr)
      htmlfile=rootname+"_wk.html"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
      groot=save_groot+"wk"
      wsm=0.5
      tab_lab=save_tab_lab+" Previous Week"
      pred_days=1 ; only look 1 month forward
      ; make obsid tstart lookup table
      mktab="tail -5000 /data/mta/DataSeeker/data/repository/ccdmfilter.rdb | awk '{print $1"+'" "'+"$3}' | sort -k2,2 -u | sort -n -k1,1 > obsid.tab"
      spawn,mktab
      readcol,"obsid.tab",wk_times,wk_obsid,format='l,l'
      ;data = data_all ; reset data
    endif ; if (n_if_time eq 3) then begin

    if (n_if_time eq 5) then begin
      qtr=where(data.time ge max(data.time)-365.*sperdy_ref)
      ;print,n_elements(data),max(data.time), sperdy_ref, qtr(0) ; debug qtr
      data=data(qtr)
      htmlfile=rootname+"_yr.html"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
      groot=save_groot+"yr"
      tab_lab=save_tab_lab+" Previous Year"
      pred_days=30 ; only look 1 month forward
      ;data = data_all ; reset data
    endif ; if (n_if_time eq 3) then begin
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      ; rework name for labelling
      if (strpos(keys(i),"_") eq 0) then s=1 else s=0
      ...
      if (xmsid ne msid) then begin
        ...
        if (xaxtime eq 1) then begin
          ...
          if (n_if_time eq 4) then begin  ; overplot obsid start times
            wk_b=where(wk_times gt xmin and wk_obsid lt 40000,n_wk_b)
            print,"obsids ",n_wk_b
            if (n_wk_b gt 0) then begin
              for iwk=0,n_wk_b-1 do begin
                oplot,[wk_times(wk_b(iwk)),wk_times(wk_b(iwk))], $
                      [ymax-0.15*(ymax-ymin),ymax], $
                      color=180,thick=0.4,linestyle=0
              endfor ; for iwk=0,n_wk_b-1 do begin
            endif ; if (n_wk_b gt 0) then begin
            ; make CAL-ER a different color
            wk_b=where(wk_times gt xmin and wk_obsid ge 40000,n_wk_b)
            print,"CAL-ER ",n_wk_b
            if (n_wk_b gt 0) then begin
              for iwk=0,n_wk_b-1 do begin
                oplot,[wk_times(wk_b(iwk)),wk_times(wk_b(iwk))], $
                      [ymax-0.08*(ymax-ymin),ymax], $
                      color=210,thick=0.4,linestyle=0
              endfor ; for iwk=0,n_wk_b-1 do begin
            endif ; if (n_wk_b gt 0) then begin
          endif ; if (n_if_time eq 4) then begin  ; overplot obsid start times
        ...
        benv=where(xtime ge xmin and xtime le xmax and  $
                   xdata ge ymin and xdata le ymax, benv_num)
        openw,elun,"env_dat.in",/get_lun
        for ienv=0,benv_num-1 do begin
          printf,elun,xtime(benv(ienv)),xdata(benv(ienv)), $
                      format='(E14.8," ",E10.4)'
        endfor
        free_lun,elun
        if (xaxtime gt 0) then begin
          case n_if_time of  ; non-time ordinate
            3: $ ; change smoothing for quarterly, one week
              spawn,"perl /data/mta4/Deriv/Script/Idl_scripts/find_moving_avg.perl env_dat.in 6.048e5 4 env_dat.out"
            4: $ ; change smoothing for weekly
              spawn,"perl /data/mta4/Deriv/Script/Idl_scripts/find_moving_avg.perl env_dat.in 8.64e4 4 env_dat.out -nodrop"
            else: $ ; smooth 1 month
              spawn,"perl /data/mta4/Deriv/Script/Idl_scripts/find_moving_avg.perl env_dat.in 2.6e6 4 env_dat.out"
          endcase ; case xax of
        endif else begin ; if (xaxtime gt 0) then begin
          case xax of  ; non-time ordinate
            'pt_suncent_ang': $
              spawn,"perl /data/mta4/Deriv/Script/Idl_scripts/find_moving_avg.perl env_dat.in 10 4 env_dat.out"
            'tephin': $
              spawn,"perl /data/mta4/Deriv/Script/Idl_scripts/find_moving_avg.perl env_dat.in 5 4 env_dat.out"
            else: begin
              fma_range=(xmax-xmin)/10
              command="perl /data/mta4/Deriv/Script/Idl_scripts/find_moving_avg.perl env_dat.in "+$
                       string(fma_range)+" 4 env_dat.out"
              spawn, command
            end ; else: begib
          endcase ; case xax of
        endelse
              
        readcol,'env_dat.out',date,avg,std,bot,mid,top,spr,bot_pt,top_pt
        if (n_if_time ne 4) then begin
          oplot,date,bot,color=blue-15,thick=2
          oplot,date,top,color=blue-15,thick=2
        ;oplot,date,bot_pt,color=blue-15,psym=2,symsize=0.6
        ;oplot,date,top_pt,color=blue-15,psym=2,symsize=0.6
        ; linear fit for envelop predictions
        if (xaxtime gt 0) then begin
          if (n_if_time eq 3) then begin ; fit 3 month for quarterly
            b=where(date gt max(date)-(90*86400L))
            scale_date=date(b)-min(date(b))
            plot_date=[min(scale_date),max(scale_date)+(30*86400L)]
          endif else begin ; fit 2 years for total
            b=where(date gt max(date)-(720*86400L))
            scale_date=date(b)-min(date(b))
            plot_date=[min(scale_date),max(scale_date)+(180*86400L)]
          endelse
          efit=linfit(scale_date,top_pt(b))
          oplot,[max(date),plot_date(1)+min(date(b))], $
              [top(n_elements(top)-1),efit(0)+plot_date(1)*efit(1)], $
              color=blue-15,thick=2,linestyle=3
          efit=linfit(scale_date,bot_pt(b))
          oplot,[max(date),plot_date(1)+min(date(b))], $
              [bot(n_elements(bot)-1),efit(0)+plot_date(1)*efit(1)], $
              color=blue-15,thick=2,linestyle=3
        endif ; if (xaxtime gt 0) then begin
        endif ; if (n_if_time ne 4)

        ;; n-degree fit for envelop predictions - in devel
        ;if (xaxtime gt 0) then begin
        ;  if (n_if_time eq 3) then begin ; fit 3 month for quarterly
        ;    b=where(date gt max(date)-(90*86400L))
        ;    scale_date=date(b)-min(date(b))
        ...
        ;    plot_date=[min(scale_date),max(scale_date)+(180*86400L)]
        ;  endelse
        ;  efit=svdfit(scale_date,top_pt(b),5)
        ;  oplot,[max(date),plot_date(1)+min(date(b))], $
        ;      [top(n_elements(top)-1),efit(0)+plot_date(1)*efit(1)+plot_date(1)^2*efit(2)+plot_date(1)^3*efit(3)+plot_date(1)^4*efit(4)], $
        ;      color=blue-15,thick=2,linestyle=3
        ;  efit=svdfit(scale_date,bot_pt(b),5)
        ;  oplot,[max(date),plot_date(1)+min(date(b))], $
        ;      [bot(n_elements(bot)-1),efit(0)+plot_date(1)*efit(1)+plot_date(1)^2*efit(2)+plot_date(1)^3*efit(3)+plot_date(1)^4*efit(4)], $
        ;      color=blue-15,thick=2,linestyle=3
        ;endif ; if (xaxtime gt 0) then begin

        ; lookup and plot limits
        lim_done=0
        ...
        for lim_loop=0,n_elements(lim_struct)-1 do begin ; start lim_loop
          ...
          if (dnum gt 0) then begin
            ...
            if (lim(0) ne -999 and xlim(0) eq -999) then begin
              ...
              if (n_if_time ne 4) then begin ; skip for weekly
              ...
              endif ;if (n_if_time ne 4) then begin ; skip for weekly
              lim_done=1
...
print, "ws ",wsmooth,ws
            ...
            ; skip for weekly oplot, xtime, df, color=blue
        ...
        if (xaxtime eq 1) then begin
          ...
          if (lim(0) ne -999 and n_if_time ne 4) then begin
            ...
            if (n_if_time ne 4) then oplot, tfit+max(xtime), ddtest, color=blue, linestyle=3
        ...
        if (n_if_time ne 4 or xaxtime eq 0) then begin ; plot derivs except for weekly
          if (xaxtime eq 1) then begin
        ...
        endif ; if (n_if_time ne 4) then begin
        if (n_if_time eq 4 and xaxtime eq 1) then begin ; plot pitch for weekly
            ymin=45
            ymax=180
            plot, alt.time,alt.pt_suncent_ang, $
                xstyle=1,ystyle=1, linestyle=0, thick=2, $
                ;title=strcompress("Pitch angle"), $
                xtitle='Time (DOY [UT])', ytitle='degrees', $
                xrange=[xmin,xmax], yrange=[ymin,ymax], $
                charsize=pcsize,charthick=1,color=white,background=bgrd, $
                xticks=nticks-1, xtickv = doyticks, xminor=10, $
                xtickformat='s2doy_axis_labels', xmargin=[xlmarg,xrmarg], $
                /nodata
            ;loadct,0
            polyfill,[xmin,xmax,xmax,xmin],[60,60,75,75],color=10,trans=1
            ;polyfill,[xmin,xmax,xmax,xmin],[75,75,90,90],color=100
            polyfill,[xmin,xmax,xmax,xmin],[90,90,110,110],color=10,trans=1
            ;polyfill,[xmin,xmax,xmax,xmin],[110,110,130,130],color=200
            polyfill,[xmin,xmax,xmax,xmin],[130,130,160,160],color=10,trans=1
            axis,yaxis=0,yrange=[ymin,ymax],ystyle=1,ytickformat='no_axis_labels'
            axis,yaxis=1,yrange=[ymin,ymax],ystyle=1,ytickformat='no_axis_labels'
            ;oplot,[xmin,xmax],[60,60],color=white,thick=0.5
            ;oplot,[xmin,xmax],[75,75],color=white,thick=0.5
            ;oplot,[xmin,xmax],[90,90],color=white,thick=0.5
            ;oplot,[xmin,xmax],[110,110],color=white,thick=0.5
            ;oplot,[xmin,xmax],[130,130],color=white,thick=0.5
            ;oplot,[xmin,xmax],[160,160],color=white,thick=0.5
            ;loadct,39
            oplot, alt.time,alt.pt_suncent_ang, $
                 linestyle=0, thick=2, color=100
            ...
            ;oplot,etime,tot_ang,color=150,linestyle=0,thick=2
            wk_b=where(wk_times gt xmin and wk_obsid lt 40000,n_wk_b)
            print,"obsids ",n_wk_b
            if (n_wk_b gt 0) then begin
              for iwk=0,n_wk_b-1 do begin
            ...
            ycon = convert_coord([0],[0], /device, /to_data)
            label_year, xmin, xmax, ycon(1), csize=1.2
            xyouts,0.3,0.49,"Sun angle",color=100,/norm,align=0.5
            ;xyouts,0.5,0.49,"Earth angle",color=150,/norm,align=0.5
            xyouts,0.7,0.49,"Altitude",color=200,/norm,align=0.5
        endif ; if (n_if_time eq 4)
      ...
      ; make html outfile
        ...
        if (lim(0) ne -999) then begin  ; color code based on limits
          ...
          ; don't check rms or deriv for hrcelec
          if (NOT strmatch(infile,"hrcelec*")) then begin
          ...
          endif ; if (NOT strmatch(infile,"hrcelec*")) then begin
        endif else begin
          if (xaxtime eq 1) then begin
        ...
        html = [html,'<tr><td><a href="javascript:WindowOpener('+ $
                    "'"+giffile+"')"+'">'+ $
                     msid+'</a></td>']
        if (abs(m(0)) gt 0.010 and abs(m(0)) lt 1000) then $
          html = [html,'<td><font color="'+mean_color+'">'+ $
                     string(format='(f8.2)',m(0))+'</font></td>'] $
        else $
          html = [html,'<td><font color="'+mean_color+'">'+ $
                     string(format='(e10.3)',m(0))+'</font></td>']
        if (abs(m(1)) gt 0.010 and abs(m(1)) lt 1000) then $
          html = [html,'<td><font color="'+rms_color+'">'+ $
                     string(format='(f8.2)',sqrt(m(1)))+'</font></td>'] $
        else $
          html = [html,'<td><font color="'+rms_color+'">'+ $
                     string(format='(e10.3)',sqrt(m(1)))+'</font></td>']
        if (abs(dfit(1)) gt 0.010 and abs(dfit(1)) lt 1000) then $
          html = [html,'<td><font color="'+d_color+'">'+ $
                     string(format='(f8.2)',dfit(1)*speryr)+'</font></td>'] $
        else $
          html = [html,'<td><font color="'+d_color+'">'+ $
                     string(format='(e10.3)',dfit(1)*speryr)+'</font></td>']
        if (abs(dfit(1)) gt 0.01 and abs(dfit(1)) lt 1000) then $
          html = [html,'<td><font color="'+dd_color+'">'+ $
                     string(format='(f8.2)',ddfit(1)*speryr*speryr)+'</font></td>'] $
        else $
          html = [html,'<td><font color="'+dd_color+'">'+ $
