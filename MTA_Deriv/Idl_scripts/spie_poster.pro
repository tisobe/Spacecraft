PRO spie_poster, infile, SIG=sig, WSMOOTH=wsmooth, PLOTX=plotx, $
...
if (keyword_set(plotx)) then begin
  ...
  set_plot, 'ps'
  device, filename='xtrend.ps',xsize=10, ysize=7.6, $
         /encapsul,/color
endelse
...
;testfor n_if_time = 1, xaxtime*3 - 1 do begin ; loop to do daily min/max
  ...
  while (loop le loop_end) do begin ; this is the loop to do several
    ...
    xdmin = min(data(where(data.tephin_avg gt 260)).(xcol))
    xdmax = max(data(where(data.tephin_avg gt 260)).(xcol))
    ...
    html = ['<tr align=center><td colspan=7>'+tab_lab+'</tr>']
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      for fit=1, filt_it do begin ; filter once by default, more on request
        ...
        ;print, m(0), sqrt(m(1)), sig ;debuggg
      ...
      if (xaxtime eq 1) then $
        plot, xtime, xdata, psym=symbol, ystyle=1, symsize=ssize, $
            title=des, xtitle='UT (Day of year)', ytitle=msid+" ("+units+")", $
            xrange=[xmin,xmax], yrange=[ymin,ymax], ylog=setlog, $
            charsize=0.4, charthick=0.8, color = white, background = bgrd, $
      ...
      if (lim(0) ne -999 and xlim(0) eq -999) then begin
        ...
        if (rnum gt 0) then oplot, xtime(r), line(r), color=red, thick=5
        if (ynum gt 0) then oplot, xtime(y), line(y), color=yellow, thick=5
        if (gnum gt 0) then oplot, xtime(g), line(g), color=green, thick=5
        lim_done=1
      ...
      if (lim_done eq 0) then oplot, xtime, line, color=blue, thick=5
      ...
      oplot, xtime, df, color=blue, thick=5
      dm=moment(dv)
      ...
      if (xaxtime eq 1) then begin
        ...
        if (lim(0) ne -999) then begin
          ...
          if (rnum gt 0) then oplot, tfit(r)+max(xtime), ddtest(r), color=red, linestyle=3, thick=5
          if (ynum gt 0) then oplot, tfit(y)+max(xtime), ddtest(y), color=yellow, linestyle=3, thick=5
          if (gnum gt 0) then oplot, tfit(g)+max(xtime), ddtest(g), color=green, linestyle=3, thick=5
        endif else begin
          oplot, tfit+max(xtime), ddtest, color=blue, linestyle=3, thick=5
      ...
      if (xaxtime eq 1) then begin
        plot, xtime, dv*speryr, ystyle=1, psym=symbol, symsize=ssize, $
            ...
            xtitle='UT (Day of year)', ytitle=units+'/Year', $
            xrange=[xmin,xmax], yrange=[ymin,ymax], $
            charsize=0.4, charthick=0.8, color = white, background = bgrd, $
            xticks=nticks-1, xtickv = doyticks, xminor=10, $
            xtickformat='s2doy_axis_labels'
        ycon = convert_coord([0],[0], /device, /to_data)
        label_year, xmin, xmax, ycon(1), csize=0.4
      endif
      if (xaxtime eq 0) then $
        plot, xtime, dv*speryr, ystyle=1, psym=symbol, symsize=ssize, $
            title=strcompress("dy/dx "+msid), $
      ...
      oplot, xtime, line, color=blue, thick=5
      giffile=strcompress(strlowcase(keys(i))+groot+ $
                             '.gif', /remove_all)
if (keyword_set(plotx)) then begin
  write_gif, outdir+"/XX"+giffile, tvrd()
endif else begin
  device, /close
endelse

print, xxx[1]  ; make it stop
     ;make html outfile
