PRO DTREND, infile, SIG=sig, WSMOOTH=wsmooth, PLOTX=plotx, $
            ...
            AVG_ONLY=avg_only, QTR_ONLY=qtr_only
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      if (xmsid ne msid) then begin
        ...
        if (n_if_time eq 1) then begin ;find daily maxes
          ...
          for k=0,n_elements(daily(0,*))-1 do begin
            ...
            if (daily(1,k) lt n_elements(xdata)) then $
              tmpdata=[tmpdata, $
                       min(where(xdata eq max(xdata(daily(0,k):daily(1,k)))))]
          endfor
        ...
        if (n_if_time eq 2) then begin ;find daily mins
          ;xtime=dailytime
          for k=0,n_elements(daily(0,*))-1 do begin
            if (daily(1,k) lt n_elements(xdata_save)) then begin
        ...
        if (xaxtime eq 1) then $
          plot, xtime, xdata, psym=symbol, xstyle=1,ystyle=1, symsize=ssize, $
        ...
        if (xaxtime eq 1) then begin
          plot, xtime, dv*speryr, xstyle=1,ystyle=1, psym=symbol, symsize=ssize, $
