PRO DTRENDF, infile, SIG=sig, WSMOOTH=wsmooth, PLOTX=plotx, $
...
if (not keyword_set(outdir)) then outdir="/data/mta4/www/DAILY/mta_deriv/F"
  ...
  while (loop le loop_end) do begin ; this is the loop to do several
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      if (xmsid ne msid) then begin
        ...
        lim.yel_hi=(lim.yel_hi-273.15)*9/5+32
        lim.yel_lo=(lim.yel_lo-273.15)*9/5+32
        lim.red_hi=(lim.red_hi-273.15)*9/5+32
        lim.red_lo=(lim.red_lo-273.15)*9/5+32
        units="F"
        ; ignore limits if not plotting versus time
        ;  (but still must call limits to set des and units
