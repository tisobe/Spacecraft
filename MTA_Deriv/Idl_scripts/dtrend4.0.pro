PRO DTREND, infile, SIG=sig, WSMOOTH=wsmooth, PLOTX=plotx, $
            ...
            AVG_ONLY=avg_only, QTR_ONLY=qtr_only, WK_ONLY=wk_only, CCODE=ccode
...
frame=["AVG","MAX","MIN","QTR","WK","YR","YR2"]
...
;testfor n_if_time = 1, 3 do begin ; loop to do daily min/max
  ...
  while (loop le loop_end) do begin ; this is the loop to do several
    ...
    tabfile=rootname+strcompress(string(loop),/remove_all)+".tab"
    if (n_if_time eq 3) then begin
      ...
      tabfile=rootname+string(loop)+"_qtr.tab"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
    ...
    if (n_if_time eq 4) then begin
      qtr=where(data.time ge max(data.time)-14*sperdy_ref)
      ;print,n_elements(data),max(data.time), sperdy_ref, qtr(0) ; debug qtr
      data=data(qtr)
      htmlfile=rootname+"_wk.html"
      tabfile=rootname+string(loop)+"_wk.tab"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
      groot=save_groot+"wk"
      wsm=0.5
    ...
    if (n_if_time eq 5) then begin
      ...
      tabfile=rootname+string(loop)+"_yr.tab"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
      groot=save_groot+"_yr"
      tab_lab=save_tab_lab+" Previous Year"
      pred_days=30 ; only look 1 month forward
      ;data = data_all ; reset data
    endif ; if (n_if_time eq 3) then begin

    if (n_if_time eq 6) then begin
      qtr=where(data.time ge max(data.time)-365.*sperdy_ref)
      ;print,n_elements(data),max(data.time), sperdy_ref, qtr(0) ; debug qtr
      data=data(qtr)
      htmlfile=rootname+"_yr_min.html"
      tabfile=rootname+string(loop)+"_yr_min.tab"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
      groot=groot+"_yr_min"
      tab_lab=tab_lab+" Daily Minimums - Past year"
    endif ; if (n_if_time eq 1) then begin

    if (n_if_time eq 1) then begin
      htmlfile=rootname+"_max.html"
      tabfile=rootname+string(loop)+"_max.tab"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
      groot=groot+"max"
    ...
    if (n_if_time eq 2) then begin
      ...
      tabfile=rootname+string(loop)+"_min.tab"
      htmlout=strcompress(outdir+"/"+htmlfile, /remove_all)
    ...
    tabfile=strcompress(tabfile,/remove_all)
    openw,TABUNIT,tabfile,/get_lun
    printf, TABUNIT, "msid mean rms first_deriv/yr first_deriv_constant sec_deriv/yr/yr sec_deriv_constant m_top b_top m_bot b_bot"
    ...
    for i = 0, n_elements(keys)-1 do begin
      ...
      if (xmsid ne msid) then begin
        ...
        if (n_if_time eq 1 or n_if_time eq 2 or n_if_time eq 6) then begin
        ...
        if (n_if_time eq 1) then begin ;find daily maxes
          ...
          openw,MAX,"daily_max.txt",/get_lun
        ...
        if (xaxtime eq 1) then begin
          plot, xtime, xdata, $
              ...
              xticks=nticks, xtickv = doyticks, xminor=10, $
        ...
        if (xaxtime eq 0) then begin
          ...
          plot, xtime, xdata, $
              ...
              ytitle=msid+" ("+units+")", $
              xrange=[xmin,xmax], yrange=[ymin,ymax], ylog=setlog, $
              ;xrange=[0.01,xmax], yrange=[0.01,ymax], /ylog,/xlog, $
...
print, "ws ",wsmooth,ws
      ...
      ; make html outfile
        ...
        printf, TABUNIT, msid,m(0), sqrt(m(1)), dfit(1)*speryr, dfit(0), $
                         ddfit(1)*speryr*speryr, ddfit(0), $
                         efit_top(1),efit_top(0),efit_bot(1),efit_bot(0), $
                         format='(A,10(" ",e9.2))'
