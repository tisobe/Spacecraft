...
if (n_elements(mthtab) gt 1) then begin
  data = 1
  fit = svdfit(intab.dist, intab.psf, fitdeg, variance=fiterr)
  fitline = fit(0)+(fit(1)*intab.dist)+(fit(2)*intab.dist*intab.dist)
...
if (data eq 1) then begin
  ...
  printf, hunit, "Below are the coefficients determined for each month.
  ...
  printf, hunit, "<tr><th>Month</th><th>Coeff2</th><th>Coeff1</th><th>Coeff0</th><th>N</th></tr>"
