PRO REJ_EVTS_OBS, rejlist, ccd, PLOTX=plotx
; given level 1 stat file, produce plots for ccd
...
b = where(data.ccd_id eq ccd)
;ccds = [0,1,2,3,4,5,6,7,8]
ccd_id = ["I0", "I1", "I2", "I3", "S0", "S1", "S2", "S3", "S4", "S5"]

plot, data(b).expno, data(b).evtsent, psym=3, charsize=1.8, $
      xtitle = "Frame", ytitle="EVTSENT", $
      title=rejlist+"  CCD"+strcompress(ccd)+"  "+ccd_id(ccd)
plot, data(b).expno, data(b).drop_amp, psym=3, charsize=1.8, $
      xtitle = "Frame", ytitle="DROP_AMP", $
      title=rejlist+"  CCD"+strcompress(ccd)+"  "+ccd_id(ccd)
plot, data(b).expno, data(b).thr_pix, psym=3, charsize=1.8, $
      xtitle = "Frame", ytitle="THR_PIX", $
      title=rejlist+"  CCD"+strcompress(ccd)+"  "+ccd_id(ccd)
plot, data(b).expno, data(b).drop_grd, psym=3, charsize=1.8, $
