pro cpix_afaint, evtlist, PLOTX=plotx, HIST=hist, TEST=test
; make corner pixel centroid plots
;   for afaint (almost faint) mode ie. VFAINT obs, but FAINT cpixs.
...
if (n_elements(apixels) gt 1) then begin
  ccds = [2,3,6,7]
  ccd_id = ["I2", "I3", "S2", "S3"]
  ...
  for i = 0, n_elements(ccds)-1 do begin
    ...
    if (n_elements(a) gt 1) then begin
      ...
      endwhile
      if (keyword_set(hist)) then begin
        write_gif, strcompress(namroot+'_'+ccd_id[i]+'ahist.gif', /remove_all), $
                   tvrd()
      endif
      ; save for below
  ...
  write_gif, strcompress(namroot+'acp.gif', /remove_all), tvrd() 
endif ;if (n_elements(apixels) gt 1) then begin
end
